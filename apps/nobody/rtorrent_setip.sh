...
echo_log "[info] Configuring rtorrent listen interface..."
...
#	else
#		echo_log "[info] VPN disabled. Setting rtorrent to listen on any interface..."
#		xmlrpc http://localhost:8080/RPC2 set_bind "0.0.0.0"
#		if [ $? -eq 0 ]; then
#			echo_log "[info] Successfully bound rtorrent to 0.0.0.0"
#		fi
	fi
	sleep 1m
done
